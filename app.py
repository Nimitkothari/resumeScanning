...
def extractPdf():
    try:
        ...
        print("name",name)
        if request.form:
            data_values = request.form.to_dict()
            emp_name = data_values['name']
            emp_id = data_values['id']
            emp_email = data_values['email']
            emp_no = data_values['phone_no']
            emp_grade = data_values['grade']
            emp_yoe = data_values['yoe']
        ...
        print("result",result)
        if request.form:
            msg = {"skills":result,"name":emp_name,"id":emp_id,"email":emp_email,"number":emp_no,"grade":emp_grade,"yoe":emp_yoe}
        else:
            msg = {"skills":result}
...
def extract():
    try:
        ...
        if len(req_body) > 1:
            ...
            emp_id = req_body['id']
            emp_email = req_body['email']
            emp_no = req_body['phone_no']
            emp_grade = req_body['grade']
            emp_yoe = req_body['yoe']
        # this gives list of words
        wordList = g_solution(text)
        print("list of words",wordList)
        # this gives me list of skills
